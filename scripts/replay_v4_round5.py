...
REPORT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "v4_round5_report.md")
...
def run_t_playback_eval(network_params, features, hist_ret, ret_daily, dates_arr, mode, eval_start_idx):
    ...
    # Mode configurations
    enable_ewma_vol = mode in ['T0', 'T2', 'T3', 'T4']
    enable_dual_bear = mode in ['T1', 'T2', 'T3', 'T4', 'F12_S4', 'F15_S4']
    fixed_vol = 0.0
    if mode in ['F10']: fixed_vol = 0.10
    if mode in ['F12', 'F12_S4']: fixed_vol = 0.12
    if mode in ['F15', 'F15_S4']: fixed_vol = 0.15
    
    ewma_multiplier = 1.0
    if mode == 'T3': ewma_multiplier = 1.25
    if mode == 'T4': ewma_multiplier = 1.50
    ...
    for step_idx in range(eval_start_idx, len(dates_arr)):
        ...
        if enable_ewma_vol:
            ...
            g_vol = min(1.0, 0.12 / sigma_risk) if sigma_risk > 0 else 1.0
            
            b_t_ewma = min(0.40, 0.40 * g_vol * ewma_multiplier)
                
            port_ret_1d = np.sum(previous_weights * ret_daily[step_idx-1]) if step_idx > 0 else 0.0
            ...
            if port_ret_1d < -0.03 or tqqq_5d < -0.15 or (sigma_21 > 0 and sigma_5_ewma / sigma_21 > 1.5):
                ...
                b_t_ewma = min(b_t_ewma, 0.10)
                
        elif fixed_vol > 0:
            b_t_ewma = fixed_vol
                
        s1_b_t_history.append(b_t_ewma)
        ...
        # Combine
        dynamic_3x_cap = min(b_t_ewma, b_t_regime)
    ...
    # FIX: Recovery Days based on actual recovery from Max DD
    # Find the earliest index after idx_max_dd where drawdown == 0
    recovery_days = 0
    is_recovered = False
    ...
    return {
        ...
        "s1_b_t_history": s1_b_t_history
    }

def main():
    print("🚀 [START] V4 Round 5: T-Series & F-Series Replay...", flush=True)
    ...
    seeds = [42, 43, 44]
    modes = ['T0', 'T1', 'T2', 'T3', 'T4', 'F10', 'F12', 'F15', 'F12_S4', 'F15_S4']
    
    md_content = "# 🛡️ QOUNT AI 第5轮：T系列与固定仓位对照实验\n\n"
    ...
    for mode in modes:
        ...
        for fold in folds:
            ...
            for seed in seeds:
                ...
                res = run_t_playback_eval(trained_params, test_seq, test_hist_ret, test_ret_daily, unique_dates, mode, eval_start_idx)
                
                if mode == 'T0':
                    s1_distribution.extend(res['s1_b_t_history'])
    ...
    md_content += "## 补充数据：T0 (S1) 日度风险预算分布\n"
    if len(s1_distribution) > 0:
        s1_arr = np.array(s1_distribution) * 100
        p10, p25, p50, p75, p90 = np.percentile(s1_arr, [10, 25, 50, 75, 90])
        std_val = np.std(s1_arr)
        min_val = np.min(s1_arr)
        max_val = np.max(s1_arr)
        
        md_content += "| P10 | P25 | P50 (Median) | P75 | P90 | Min | Max | Std Dev |\n"
        md_content += "|---|---|---|---|---|---|---|---|\n"
        md_content += f"| {p10:.1f}% | {p25:.1f}% | {p50:.1f}% | {p75:.1f}% | {p90:.1f}% | {min_val:.1f}% | {max_val:.1f}% | {std_val:.2f}% |\n\n"

    with open(REPORT_PATH, "w") as f:
        f.write(md_content)
        
    print(f"\n✅ Round 5 T-Series Complete. Report at: {REPORT_PATH}", flush=True)
