...
REPORT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "v4_grid2_report.md")
...
def evaluate_model_detailed(network_params, test_env, test_params):
    ...
    def _eval_step(state, step_idx):
        ...
        tech_w = weights[0] + weights[1]
        tech_scale = jnp.where(tech_w > test_params.max_tech_weight, test_params.max_tech_weight / (tech_w + 1e-8), 1.0)
        weights = weights.at[0].mul(tech_scale)
        weights = weights.at[1].mul(tech_scale)
        
        total_3x_w = jnp.sum(weights[:8])
        total_scale = jnp.where(total_3x_w > test_params.max_total_3x, test_params.max_total_3x / (total_3x_w + 1e-8), 1.0)
        weights = weights.at[:8].mul(total_scale)
    ...
    # The max weight across all 3x assets (0 to 7)
    ...
    # Missing Metric 1: Worst 21-day return
    ...
    # Missing Metric 2: CAGR without Top 10 days
    ...
    # Missing Metric 3: Drawdown Recovery Time
    new_high_idx = np.where(drawdowns == 0)[0]
    if len(new_high_idx) > 1:
        gaps = np.diff(new_high_idx)
        max_recovery_days = np.max(gaps)
    else:
        max_recovery_days = len(port_vals_arr)
...
def run_experiment(df, assets, base_features, cand, fold, seed):
    ...
    train_env.default_params = EnvParams(
        ...
        max_tech_weight=cand['max_tech'],
        max_total_3x=cand['max_3x']
    )
...
def main():
    print("🚀 [START] V4 Grid 2: Constraints Matrix...", flush=True)
    ...
    candidates = [
        # Part 1: Single Asset Caps
        {"name": "W1 (Max 20%)", "max_w": 0.20, "max_tech": 1.0, "max_3x": 1.0},
        {"name": "W2 (Max 25%)", "max_w": 0.25, "max_tech": 1.0, "max_3x": 1.0},
        {"name": "W3 (Max 30%)", "max_w": 0.30, "max_tech": 1.0, "max_3x": 1.0},
        {"name": "W4 (Max 40%)", "max_w": 0.40, "max_tech": 1.0, "max_3x": 1.0},
        
        # Part 2: Portfolio Level Caps
        {"name": "G1 (W=30%, 3x=50%, Tech=30%)", "max_w": 0.30, "max_tech": 0.30, "max_3x": 0.50},
        {"name": "G2 (W=30%, 3x=40%, Tech=30%)", "max_w": 0.30, "max_tech": 0.30, "max_3x": 0.40},
        {"name": "G3 (W=30%, 3x=60%, Tech=40%)", "max_w": 0.30, "max_tech": 0.40, "max_3x": 0.60}
    ]
    
    md = "# 🛡️ QOUNT AI V4 第二轮网格：执行层组合仓位上限深度测试\n\n"
    md += "固定 Deadband=0%，测试单资产上限 (20%~40%) 以及组合级风险限额 (3倍ETF总量限制、科技簇限制)。\n\n"
    
    for cand in candidates:
        print(f"\n⚙️ Testing Candidate: {cand['name']}", flush=True)
        md += f"## {cand['name']}\n"
        md += "| 周期 | Seed | CAGR | Max DD | 恢复期 | 最差21日 | 扣Top10 CAGR | Sharpe | 换手率 | 最大单风险 | 平均3x | 平均科技 |\n"
        md += "|---|---|---|---|---|---|---|---|---|---|---|---|\n"
        ...
        for fold in folds:
            ...
            for seed in seeds:
                ...
                print(f"     -> Seed {seed} | CAGR: {res['CAGR']:.2f}% | DD: {res['Max DD']:.2f}% | Rec: {res['Recovery Days']}d", flush=True)
    ...
    print(f"\n✅ Grid 2 Complete. Report at: {REPORT_PATH}", flush=True)
