...
MODEL_OUT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "models", "jax_live_ppo.pkl")
...
def make_train_fn(env: JaxPortfolioEnv, hp_lr=5e-4, hp_ent=0.02, total_timesteps=8_000_000, num_envs=64, num_steps=256):
...
def main():
    print("Loading data for LIVE TRAINING (Using ALL data)...")
    df = pd.read_parquet(DATA_PATH)
    
    def prepare_data_live(df_part):
        ...
        # Feature Normalization using ALL data
        ...
        return feat_array, pivot_ret.loc[common_dates][assets].values, feat_mean, feat_std, pivot_feat.columns, assets

    # DO NOT split. Use all data for live model.
    all_feat, all_ret, mean_params, std_params, feat_cols, assets_list = prepare_data_live(df)
    
    print(f"JAX Device: {jax.devices()}")
    
    env = JaxPortfolioEnv(all_feat, all_ret, penalty_enabled=True, slip_rate=0.002)
    
    # We use the best hyperparams: Balanced_Search (LR=0.0005, Ent=0.02)
    # We also boost steps to 8 Million since the dataset is much larger (includes 2022-2024).
    print(f"\n🚀 Launching Final Live Model Training (8M steps)")
    start_time = time.time()
    train_fn = make_train_fn(env, hp_lr=0.0005, hp_ent=0.02, total_timesteps=8_000_000)
    
    jit_train = jax.jit(train_fn)
    trained_params = jit_train(jax.random.PRNGKey(888))
    
    end_time = time.time()
    print(f"✅ Live Model Trained in {end_time - start_time:.2f} seconds")
    
    # Export weights & scaler logic to Python dict
    # Extract native numpy arrays from JAX DeviceArray
    weights_dict = jax.tree_util.tree_map(lambda x: np.array(x), trained_params)
    
    export_payload = {
        "model_weights": weights_dict,
        "scaler_mean": mean_params,
        "scaler_std": std_params,
        "assets": assets_list,
    ...
    print(f"💾 Live Model weights & scaler saved to {MODEL_OUT_PATH}")
