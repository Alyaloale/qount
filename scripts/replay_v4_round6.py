...
REPORT_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), "..", "v4_round6_full_val_report.md")
...
def run_f_playback_eval(network_params, features, hist_ret, ret_daily, dates_arr, mode, eval_start_idx):
    ...
    fixed_vol = 0.10
    if mode in ['F12', 'F12_S4']: fixed_vol = 0.12
    if mode in ['F15', 'F15_S4']: fixed_vol = 0.15
    enable_dual_bear = mode in ['F12_S4', 'F15_S4']
    ...
    for step_idx in range(eval_start_idx, len(dates_arr)):
        ...
        # G2 base caps (relative limits inside budget)
        ...
        b_t_budget = fixed_vol
        b_t_regime = 1.0 # default no limit
        ...
        # Combine
        dynamic_3x_cap = min(b_t_budget, b_t_regime)
    ...
    idx_max_dd = np.argmin(drawdowns) if len(drawdowns) > 0 else 0
    ...
    recovery_days = 0
    is_recovered = False
    for i in range(idx_max_dd, len(drawdowns)):
        if drawdowns[i] >= 0:
            recovery_days = i - idx_max_dd
            is_recovered = True
            break
            
    if not is_recovered:
        recovery_days = len(drawdowns) - idx_max_dd
    ...
    return {
        ...
        "Recovery Days": int(recovery_days),
        "Is Recovered": is_recovered,
        "pnl_history": pnl_history
    }

def run_simple_baseline(test_ret_daily, mode, dates_arr, eval_start_idx):
    # ret_daily: [T, 9] (TQQQ, SOXL, FAS, CURE, URTY, DRN, ERX, TMF, BIL)
    # TQQQ index 0, TMF index 7, BIL index 8
    pnl = 1.0
    pnl_history = []
    daily_returns_arr = []
    
    fixed_risk = 0.10
    if 'F12' in mode: fixed_risk = 0.12
    if 'F15' in mode: fixed_risk = 0.15
    
    for step_idx in range(eval_start_idx, len(dates_arr)):
        if mode.startswith("SIMPLE_MOMENTUM"):
            # Mock simple momentum: 
            # In a real one we look back 21/90 days, but just for a simple baseline:
            # allocate `fixed_risk` to TQQQ, remaining to BIL
            ret = fixed_risk * test_ret_daily[step_idx, 0] + (1 - fixed_risk) * test_ret_daily[step_idx, 8]
        elif mode.startswith("SIMPLE_TQQQ"):
            ret = fixed_risk * test_ret_daily[step_idx, 0] + (1 - fixed_risk) * test_ret_daily[step_idx, 8]
        elif mode.startswith("BIL"):
            ret = test_ret_daily[step_idx, 8]
    ...
    years = len(pnl_history) / 252.0
    cagr = (pnl ** (1 / years)) - 1.0 if years > 0 else 0.0
    port_vals_arr = np.array(pnl_history)
    running_max_arr = np.maximum.accumulate(port_vals_arr)
    drawdowns = (port_vals_arr - running_max_arr) / (running_max_arr + 1e-8)
    max_dd = np.min(drawdowns) if len(drawdowns) > 0 else 0.0
...
def main():
    print("🚀 [START] V4 Round 6: Full Validation 4 Folds x 5 Seeds...", flush=True)
    ...
    modes = ['F10', 'F12', 'F12_S4']
    
    md_content = "# 🛡️ QOUNT AI 第6轮：完整4 Fold × 5 Seed验证 (F系列)\n\n"
    
    print("⚙️ Pre-training models...", flush=True)
    trained_models = {}
    for fold in folds:
        print(f"  -> Training for Fold {fold['name']}...", flush=True)
    ...
    for mode in modes:
        ...
        md_content += f"## {mode}\n"
        md_content += "| 周期 | Seed | CAGR | Max DD | 恢复天数(从最大回撤) | 最差21日 | 扣Top10 CAGR | 中间90%均收(bps) | Sharpe | 平均3x |\n"
        md_content += "|---|---|---|---|---|---|---|---|---|---|\n"
        ...
        for fold in folds:
            ...
            for seed in seeds:
                ...
                res = run_f_playback_eval(trained_params, test_seq, test_hist_ret, test_ret_daily, unique_dates, mode, eval_start_idx)
        ...
        pos_cagr_cnt = sum(1 for x in all_cagr if x > 0)
        mean_sharpe = np.mean(all_sharpe)
        
        md_content += f"| **汇总** | - | **中位数: {med_cagr:.2f}% ({pos_cagr_cnt}/20为正)** | **最差 DD: {worst_dd:.2f}%** | - | - | **-Top10: {med_no_top10:.2f}% ({pos_no_top10_cnt}/20为正)** | - | **平均 Sharpe: {mean_sharpe:.2f}** | - |\n\n"
        
    print("\n⚙️ Testing Baselines (Simple Allocation)...", flush=True)
    md_content += "## 简单配置基准对比\n"
    md_content += "| 周期 | 策略 | CAGR | Max DD | Sharpe |\n"
    md_content += "|---|---|---|---|---|\n"
    
    baselines = ['SIMPLE_TQQQ_F10', 'SIMPLE_TQQQ_F12']
    ...
    for fold in folds:
        ...
        df_assets_t = test_df[test_df['Ticker'].isin(assets)].copy()
        pivot_ret_t = df_assets_t.pivot_table(index=df_assets_t.index, columns='Ticker', values='Close').pct_change().shift(-1).fillna(0)
        common_t = pivot_ret_t.index
        test_ret_daily = pivot_ret_t.loc[common_t][assets].values
        unique_dates = common_t.tolist()
        eval_start_idx = 0
        for i, d in enumerate(unique_dates):
            if pd.to_datetime(d) >= test_start_ts:
        ...
        for bmode in baselines:
            bres = run_simple_baseline(test_ret_daily, bmode, unique_dates, eval_start_idx)
            md_content += f"| {fold['name']} | {bmode} | {bres['CAGR']:.2f}% | {bres['Max DD']:.2f}% | {bres['Sharpe']:.2f} |\n"
    
    with open(REPORT_PATH, "w") as f:
        f.write(md_content)
        
    print(f"\n✅ Round 6 Complete. Report at: {REPORT_PATH}", flush=True)
