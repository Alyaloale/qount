#!/usr/bin/env python3
"""Run direct economic ML with point-in-time BTC/ETH DVOL features."""
...
from qount.mini_trend.regime_dvol_ml import build_dvol_economic_ml_matrix  # noqa: E402
from qount.mini_trend.regime_dvol_ml import write_dvol_economic_ml_artifact  # noqa: E402
...
def main(argv: list[str] | None = None) -> int:
    ...
    dvol_payload = json.loads(Path(args.dvol_path).read_text(encoding="utf-8"))
    config = EconomicMLConfig(
        ...
        prior_family_trial_count=53,
    )
    payload = build_dvol_economic_ml_matrix(
        bars, funding, dvol_payload, config, use_gpu=args.use_gpu
    )
    artifact = write_dvol_economic_ml_artifact(
