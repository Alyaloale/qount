#!/usr/bin/env python3
"""Run the frozen downside-only risk rules on the retained on-chain ranker."""
...
from qount.mini_trend.regime_onchain_strategy import (  # noqa: E402
    build_onchain_strategy_ablation,
)
from qount.mini_trend.regime_onchain_strategy import (  # noqa: E402
    write_onchain_strategy_ablation_artifact,
)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--onchain-model-path", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    model = json.loads(Path(args.onchain_model_path).read_text(encoding="utf-8"))
    payload = build_onchain_strategy_ablation(bars, funding, rules, model)
    artifact = write_onchain_strategy_ablation_artifact(
