#!/usr/bin/env python3
"""Preregister or run formal crypto trial 147 (layered state machine) on existing TOP3 UM daily data."""
...
from qount.mini_trend.multi_speed_state_machine import (  # noqa: E402
...
DEFAULT_OUTPUT_ROOT = ROOT / "state" / "research_governance" / "crypto_state_machine"
_DAY_MS = 86_400_000
...
def _write_once(path: Path, value: object) -> None:
    ...
    try:
        ...
        if path.read_bytes() != raw:
            raise RuntimeError(f"state_machine_readback_mismatch:{path}")
        if stat.S_IMODE(path.stat().st_mode) != 0o600:
            raise RuntimeError(f"state_machine_mode_invalid:{path}")
    finally:
        if fd >= 0:
            os.close(fd)
        if tmp.exists():
...
def _verify_bundle(bundle_dir: Path) -> dict[str, Any]:
    ...
    if manifest["manifest_hash"] != canonical_hash(core):
        raise RuntimeError("state_machine_manifest_hash_invalid")
    for member in manifest["member_files"]:
        ...
        if len(raw) != member["size_bytes"] or hashlib.sha256(raw).hexdigest() != member["sha256"]:
            raise RuntimeError(f"state_machine_member_hash_invalid:{member['path']}")
...
def _preregister(output_root: Path) -> Path:
    ...
    if path.exists():
        ...
        validate_state_machine_preregistration(existing)
        return path
    payload = build_state_machine_preregistration(datetime.now(timezone.utc).isoformat())
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    validate_state_machine_preregistration(prereg)
    ...
    code_paths = (
        Path("src/qount/mini_trend/multi_speed_state_machine.py"),
        ...
        Path("scripts/research/mini_trend/run_crypto_state_machine_trial.py"),
    )
    ...
    experiment = GlobalExperimentRecord.create(
        hypothesis_family=STATE_MACHINE_PROTOCOL.hypothesis_family,
        trial_number_within_family=STATE_MACHINE_PROTOCOL.trial_number_within_family,
        research_question=(
            "Does a layered state machine -- slow regime gate, medium position "
            "selection, fast asymmetric de-risk -- improve TOP3 standalone CAGR "
            "versus Base v0.2 without materially worsening drawdown?"
        ),
        economic_mechanism=(
            "Assigning each trend speed a distinct hierarchical role (regime, "
            "selection, de-risk) rather than combining them into one signal may "
            "capture different trend phases while reducing exposure asynchronously "
            "when short-term momentum reverses."
        ),
        baseline_ids=("MiniTrend-UM-Base-v0.2", "cash_zero_return"),
        preregistered_primary_metric=STATE_MACHINE_PROTOCOL.protocol_basis["primary_metric"],
        preregistered_failure_conditions=STATE_MACHINE_PROTOCOL.protocol_basis["failure_conditions"],
        allowed_sensitivity_range=STATE_MACHINE_PROTOCOL.protocol_basis["allowed_sensitivity_range"],
        dataset_ids=(report["data"]["data_hash"],),
        data_role="consumed_historical_discovery_pool",
        untouched_data_ids=(),
        code_hash=code_hash,
        config_hash=STATE_MACHINE_PROTOCOL.protocol_hash,
        source_hashes=source_hashes,
        first_result_observed_at=observed_at,
        reviewer_observations=({
            "role": "automated",
            "at": observed_at,
            "note": "Formal global trial 147 (3/3) completed; family review triggered.",
        },),
        ...
        contamination_notes=(
            ...
            "Trial 145/146 parameters remain frozen; this is the final trial before review.",
        ),
    )
    errors = experiment.validate()
    if errors:
        raise RuntimeError(f"state_machine_experiment_record_invalid:{','.join(errors)}")

    bundle_core = {
        "schema_version": 1,
        "artifact_type": "crypto_multi_speed_state_machine_bundle",
        "observed_at": observed_at,
        "global_trial_number": STATE_MACHINE_PROTOCOL.global_trial_number,
        "trial_number_within_family": STATE_MACHINE_PROTOCOL.trial_number_within_family,
        "hypothesis_family": STATE_MACHINE_PROTOCOL.hypothesis_family,
        "contract_hash": STATE_MACHINE_PROTOCOL.contract_hash,
        "protocol_hash": STATE_MACHINE_PROTOCOL.protocol_hash,
        "data_hash": report["data"]["data_hash"],
