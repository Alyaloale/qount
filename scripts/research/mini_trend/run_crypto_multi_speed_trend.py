#!/usr/bin/env python3
"""Preregister or run formal crypto trial 145 on existing TOP3 UM daily data."""
...
from qount.mini_trend.multi_speed_trend import (  # noqa: E402
    MULTI_SPEED_TREND_PROTOCOL,
    build_multi_speed_preregistration,
    build_multi_speed_report,
    validate_multi_speed_preregistration,
)


DEFAULT_KLINE_CACHE = ROOT / "state" / "r0_runtime" / "klines"
DEFAULT_FUNDING_CACHE = ROOT / "state" / "r0_runtime" / "funding"
DEFAULT_OUTPUT_ROOT = ROOT / "state" / "research_governance" / "crypto_multi_speed_trend"
_DAY_MS = 86_400_000
...
def _write_once(path: Path, value: object) -> None:
    ...
    try:
        ...
        if path.read_bytes() != raw:
            raise RuntimeError(f"multi_speed_readback_mismatch:{path}")
        if stat.S_IMODE(path.stat().st_mode) != 0o600:
            raise RuntimeError(f"multi_speed_mode_invalid:{path}")
    finally:
        if fd >= 0:
            os.close(fd)
        if tmp.exists():
...
def _verify_bundle(bundle_dir: Path) -> dict[str, Any]:
    ...
    if manifest["manifest_hash"] != canonical_hash(core):
        raise RuntimeError("multi_speed_manifest_hash_invalid")
    for member in manifest["member_files"]:
        ...
        if len(raw) != member["size_bytes"] or hashlib.sha256(raw).hexdigest() != member["sha256"]:
            raise RuntimeError(f"multi_speed_member_hash_invalid:{member['path']}")
...
def _funding_audit(bars_by_symbol: dict, funding_by_symbol: dict) -> dict[str, Any]:
    ...
    warmup = max(
        ...
        MULTI_SPEED_TREND_PROTOCOL.slow_gate_lookback,
    )
...
def _preregister(output_root: Path) -> Path:
    ...
    if path.exists():
        ...
        validate_multi_speed_preregistration(existing)
        return path
    payload = build_multi_speed_preregistration(datetime.now(timezone.utc).isoformat())
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    validate_multi_speed_preregistration(prereg)
    ...
    report, trajectories = build_multi_speed_report(
        bars, funding, prereg,
    ...
    code_paths = (
        ...
        Path("scripts/research/mini_trend/run_crypto_multi_speed_trend.py"),
    )
    ...
    experiment = GlobalExperimentRecord.create(
        hypothesis_family=MULTI_SPEED_TREND_PROTOCOL.hypothesis_family,
        trial_number_within_family=MULTI_SPEED_TREND_PROTOCOL.trial_number_within_family,
        research_question=(
            "Does fixed 20/60/120-day direction consistency improve TOP3 standalone CAGR "
            "versus Base v0.2 without materially worsening drawdown?"
        ),
        economic_mechanism=(
            "Agreement across fast, medium, and slow trailing returns filters transient trend "
            "noise while retaining long-only exposure to persistent crypto trends."
        ),
        baseline_ids=("MiniTrend-UM-Base-v0.2", "cash_zero_return"),
        preregistered_primary_metric=MULTI_SPEED_TREND_PROTOCOL.protocol_basis["primary_metric"],
        preregistered_failure_conditions=MULTI_SPEED_TREND_PROTOCOL.protocol_basis["failure_conditions"],
        allowed_sensitivity_range=MULTI_SPEED_TREND_PROTOCOL.protocol_basis["allowed_sensitivity_range"],
        dataset_ids=(report["data"]["data_hash"],),
        data_role="consumed_historical_discovery_pool",
        untouched_data_ids=(),
        code_hash=code_hash,
        config_hash=MULTI_SPEED_TREND_PROTOCOL.protocol_hash,
        source_hashes=source_hashes,
        first_result_observed_at=observed_at,
        reviewer_observations=({
            "role": "automated",
            "at": observed_at,
            "note": "Formal global trial 145 completed from the frozen preregistration.",
        },),
    ...
    if errors:
        raise RuntimeError(f"multi_speed_experiment_record_invalid:{','.join(errors)}")

    bundle_core = {
        "schema_version": 1,
        "artifact_type": "crypto_multi_speed_trend_bundle",
        "observed_at": observed_at,
        "global_trial_number": MULTI_SPEED_TREND_PROTOCOL.global_trial_number,
        "trial_number_within_family": MULTI_SPEED_TREND_PROTOCOL.trial_number_within_family,
        "hypothesis_family": MULTI_SPEED_TREND_PROTOCOL.hypothesis_family,
        "contract_hash": MULTI_SPEED_TREND_PROTOCOL.contract_hash,
        "protocol_hash": MULTI_SPEED_TREND_PROTOCOL.protocol_hash,
        "data_hash": report["data"]["data_hash"],
