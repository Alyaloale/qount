#!/usr/bin/env python3
"""Preregister or run formal trial 148 (crypto_vol_crisis_state_v1) on existing TOP3 UM daily data."""
...
from qount.mini_trend.vol_crisis_state import (  # noqa: E402
...
DEFAULT_OUTPUT_ROOT = ROOT / "state" / "research_governance" / "crypto_vol_crisis"
_DAY_MS = 86_400_000
...
def _write_once(path: Path, value: object) -> None:
    ...
    try:
        ...
        if path.read_bytes() != raw:
            raise RuntimeError(f"crisis_state_readback_mismatch:{path}")
        if stat.S_IMODE(path.stat().st_mode) != 0o600:
            raise RuntimeError(f"crisis_state_mode_invalid:{path}")
    finally:
        if fd >= 0:
            os.close(fd)
        if tmp.exists():
...
def _verify_bundle(bundle_dir: Path) -> dict[str, Any]:
    ...
    if manifest["manifest_hash"] != canonical_hash(core):
        raise RuntimeError("crisis_state_manifest_hash_invalid")
    for member in manifest["member_files"]:
        ...
        if len(raw) != member["size_bytes"] or hashlib.sha256(raw).hexdigest() != member["sha256"]:
            raise RuntimeError(f"crisis_state_member_hash_invalid:{member['path']}")
...
def _funding_audit(bars_by_symbol: dict, funding_by_symbol: dict) -> dict[str, Any]:
    ...
    warmup = max(frozen_top3_config().gate_sma, frozen_top3_config().trend_sma, frozen_top3_config().slow_sma)
...
def _preregister(output_root: Path) -> Path:
    ...
    if path.exists():
        ...
        validate_crisis_state_preregistration(existing)
        return path
    payload = build_crisis_state_preregistration(datetime.now(timezone.utc).isoformat())
...
def _run(args: argparse.Namespace) -> dict[str, Any]:
    ...
    validate_crisis_state_preregistration(prereg)
    ...
    code_paths = (
        Path("src/qount/mini_trend/vol_crisis_state.py"),
        Path("src/qount/mini_trend/multi_speed_trend.py"),
        Path("src/qount/mini_trend/futures_recovery_backtest.py"),
        Path("scripts/research/mini_trend/run_vol_crisis_state_trial.py"),
    )
    ...
    experiment = GlobalExperimentRecord.create(
        hypothesis_family=CRISIS_STATE_PROTOCOL.hypothesis_family,
        trial_number_within_family=CRISIS_STATE_PROTOCOL.trial_number_within_family,
        research_question=(
            "Does a three-signal crisis RiskMultiplier (downside semi-variance, "
            "BTC-alt correlation surge, funding extremes) improve drawdown without "
            "excessive return sacrifice across Base, multi-speed and vol-target baselines?"
        ),
        economic_mechanism=(
            "A composite crisis z-score from non-price-only signals (funding extremes, "
            "BTC-alt correlation surge, downside semi-variance) maps to a continuous "
            "RiskMultiplier [0,1] that overlays existing strategies' position sizes."
        ),
        baseline_ids=("MiniTrend-UM-Base-v0.2", "multi_speed_direction_consistency_v1", "fixed_vol_target"),
        preregistered_primary_metric=CRISIS_STATE_PROTOCOL.protocol_basis["primary_metric"],
        preregistered_failure_conditions=CRISIS_STATE_PROTOCOL.protocol_basis["failure_conditions"],
        allowed_sensitivity_range=CRISIS_STATE_PROTOCOL.protocol_basis["allowed_sensitivity_range"],
        dataset_ids=(report["data"]["data_hash"],),
        data_role="consumed_historical_discovery_pool",
        untouched_data_ids=(),
        code_hash=code_hash,
        config_hash=CRISIS_STATE_PROTOCOL.protocol_hash,
        source_hashes=source_hashes,
        first_result_observed_at=observed_at,
        reviewer_observations=({
            "role": "automated",
            "at": observed_at,
            "note": "Formal global trial 148 (crypto_vol_crisis_state_v1 trial 1/3).",
        },),
        ...
        contamination_notes=(
            ...
            "Distinct from price-only vol/HMM/GRU and Trial 147 fast de-risk.",
        ),
    )
    errors = experiment.validate()
    if errors:
        raise RuntimeError(f"crisis_state_experiment_record_invalid:{','.join(errors)}")

    bundle_core = {
        "schema_version": 1,
        "artifact_type": "crypto_vol_crisis_state_bundle",
        "observed_at": observed_at,
        "global_trial_number": CRISIS_STATE_PROTOCOL.global_trial_number,
        "trial_number_within_family": CRISIS_STATE_PROTOCOL.trial_number_within_family,
        "hypothesis_family": CRISIS_STATE_PROTOCOL.hypothesis_family,
        "contract_hash": CRISIS_STATE_PROTOCOL.contract_hash,
        "protocol_hash": CRISIS_STATE_PROTOCOL.protocol_hash,
        "data_hash": report["data"]["data_hash"],
    ...
    return {
        ...
        "crisis_comparison": [
            {
                "baseline": c["baseline"],
                "drawdown_saved": round(c["drawdown_saved"], 4),
                "return_sacrificed": round(c["return_sacrificed"], 4),
                "ratio": round(c["drawdown_saved_per_return_sacrificed"], 4) if c["drawdown_saved_per_return_sacrificed"] != float("inf") else "inf",
            }
            for c in report["crisis_comparison"]
        ],
        "candidate_pnl_ready": False,
        "orders_authorized": False,
    }
