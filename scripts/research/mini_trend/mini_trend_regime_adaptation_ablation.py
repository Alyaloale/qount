#!/usr/bin/env python3
"""Run the fixed downside-only strategy ablation for the confirmed rolling ranker."""
...
from qount.mini_trend.regime_adaptation_ablation import (  # noqa: E402
    build_adaptation_risk_ablation,
)
from qount.mini_trend.regime_adaptation_ablation import (  # noqa: E402
    write_adaptation_risk_ablation_artifact,
)
...
def main(argv: list[str] | None = None) -> int:
    ...
    confirmation = json.loads(Path(args.confirmation_path).read_text(encoding="utf-8"))
    rules = json.loads(Path(args.exchange_rules_path).read_text(encoding="utf-8"))
    payload = build_adaptation_risk_ablation(bars, funding, rules, confirmation)
    artifact = write_adaptation_risk_ablation_artifact(
