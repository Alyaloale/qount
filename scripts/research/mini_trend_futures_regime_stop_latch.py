#!/usr/bin/env python3
"""Preregister or run the no-threshold UM regime stop-latch diagnostic."""
...
from qount.mini_trend.futures_regime_stop_latch import (  # noqa: E402
    FUTURES_REGIME_STOP_LATCH_PROTOCOL,
    build_regime_stop_latch_preregistration,
    write_regime_stop_latch_preregistration_artifact,
)
from qount.mini_trend.futures_regime_stop_latch_report import (  # noqa: E402
    build_regime_stop_latch_historical_report,
    write_regime_stop_latch_historical_artifact,
)
...
def parse_args(argv: list[str]) -> argparse.Namespace:
    ...
    parser.add_argument("--prior-regime-overlay-path", required=True)
...
def main(argv: list[str] | None = None) -> int:
    ...
    evidence = {
        ...
        "prior_regime_overlay_path": args.prior_regime_overlay_path,
    }
    if args.run:
        if not args.preregistration_path:
            raise ValueError("--run requires --preregistration-path")
        payload = build_regime_stop_latch_historical_report(
            _historical_inputs(args.cache_dir),
            rules,
            _load_object(args.preregistration_path),
            **evidence,
        )
        artifact = write_regime_stop_latch_historical_artifact(
            Settings.from_env(), payload, explicit_path=args.output_path
        )
    else:
        payload = build_regime_stop_latch_preregistration(rules, **evidence)
        artifact = write_regime_stop_latch_preregistration_artifact(
            Settings.from_env(), payload, explicit_path=args.output_path
        )
