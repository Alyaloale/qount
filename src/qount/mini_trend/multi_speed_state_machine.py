"""Trial 147: layered multi-speed state machine for the TOP3 UM research line.

This is the third and final formal trial of ``multi_speed_trend_v1`` before
mandatory family review. It is a distinct mechanism from Trial 145's
direction-consistency vote and Trial 146's continuous z-score composite: each
speed has a separate hierarchical role rather than being combined into one
signal.

- Slow layer (120-day): regime gate -- BTC positive or 2/3 breadth -> risk-on.
- Medium layer (60-day): position selection -- symbol eligible if 60-day > 0.
- Fast layer (20-day): asymmetric de-risk -- symbol weight halved if 20-day < 0.

The fast layer only reduces risk; it never adds exposure (that is the medium
layer's role). Sizing uses the same vol-inverse / correlation / ATR framework
as Trials 145/146 so the comparison isolates the state-machine mechanism.
"""
...
STATE_MACHINE_PREREG_VERSION = "crypto_multi_speed_state_machine_preregistration_v0.1"
STATE_MACHINE_REPORT_VERSION = "crypto_multi_speed_state_machine_trial_v0.1"


@dataclass(frozen=True)
class MultiSpeedStateMachineProtocol:
    hypothesis_family: str = "multi_speed_trend_v1"
    candidate_id: str = "multi_speed_layered_state_machine_v1"
    global_trial_number: int = 147
    trial_number_within_family: int = 3
    ...
    slow_regime_lookback: int = 120
    slow_regime_breadth: float = 2.0 / 3.0
    medium_position_lookback: int = 60
    fast_derisk_lookback: int = 20
    fast_derisk_factor: float = 0.5
    vol_lookback: int = 20
    atr_lookback: int = 14
    ...
    def contract_basis(self) -> dict[str, Any]:
        return {
            ...
            "signal": {
                "architecture": "layered_state_machine",
                "slow_regime": {
                    "lookback": self.slow_regime_lookback,
                    "gate": "BTC positive OR 2/3 breadth positive",
                    "role": "risk_on_off_gate",
                },
                "medium_position": {
                    "lookback": self.medium_position_lookback,
                    "gate": "symbol trailing return positive",
                    "role": "eligibility_and_base_allocation",
                },
                "fast_derisk": {
                    "lookback": self.fast_derisk_lookback,
                    "gate": "symbol trailing return negative",
                    "factor": self.fast_derisk_factor,
                    "role": "asymmetric_risk_reduction_only",
                },
    ...
    def protocol_basis(self) -> dict[str, Any]:
        return {
            ...
            "interpretation": (
                "Formal discovery trial 147 and final trial of multi_speed_trend_v1 "
                "before mandatory family review. A rejection closes the family or "
                "requires a different economic mechanism; it does not rescue Trials "
                "145 or 146."
            ),
...
STATE_MACHINE_PROTOCOL = MultiSpeedStateMachineProtocol()


def build_state_machine_preregistration(created_at: str) -> dict[str, Any]:
    protocol = STATE_MACHINE_PROTOCOL
    return {
        "schema_version": STATE_MACHINE_PREREG_VERSION,
        "artifact_type": "crypto_multi_speed_state_machine_preregistration",
...
def validate_state_machine_preregistration(payload: Mapping[str, Any]) -> None:
    protocol = STATE_MACHINE_PROTOCOL
    if payload.get("schema_version") != STATE_MACHINE_PREREG_VERSION:
        raise ValueError("unexpected state-machine preregistration schema")
    ...
    if (
        ...
        raise ValueError("state-machine contract hash mismatch")
    ...
    if (
        ...
        raise ValueError("state-machine protocol hash mismatch")
    if payload.get("meta", {}).get("strategy_results_evaluated") is not False:
        raise ValueError("state-machine preregistration contains evaluated results")


def research_rules() -> dict[str, dict[str, float]]:
    floor = STATE_MACHINE_PROTOCOL.estimated_min_notional_usdt
...
class StateMachineSelector:
    ...
    def __call__(
        ...
        required = max(
            protocol.slow_regime_lookback,
            protocol.medium_position_lookback,
            protocol.fast_derisk_lookback,
        ...
        # Layer 1: slow regime gate (120-day)
        slow_positive = {
            symbol: _trailing_return(signal_bars[symbol], protocol.slow_regime_lookback) > 0.0
            for symbol in TOP3
        }
        regime_on = slow_positive["BTCUSDT"] or (
            sum(slow_positive.values()) / len(TOP3) >= protocol.slow_regime_breadth
        )
        if not regime_on:
            return {symbol: 0.0 for symbol in TOP3}, "cash_slow_regime_off", True

        # Layer 2: medium position selection (60-day)
        returns_by_symbol = {
        ...
        derisk_factors: dict[str, float] = {}
        for symbol in TOP3:
            medium_return = _trailing_return(signal_bars[symbol], protocol.medium_position_lookback)
            if medium_return <= 0.0:
                continue
            ...
            # Layer 3: fast de-risk (20-day) -- asymmetric, only reduces
            fast_return = _trailing_return(signal_bars[symbol], protocol.fast_derisk_lookback)
            derisk_factors[symbol] = protocol.fast_derisk_factor if fast_return < 0.0 else 1.0

        if not raw:
            return {symbol: 0.0 for symbol in TOP3}, "cash_no_medium_eligible", True
        raw_total = sum(raw.values())
        proposed = {
            symbol: raw[symbol] / raw_total * scales[symbol] * derisk_factors[symbol]
            for symbol in raw
        }
        ...
        return allocated, "multi_speed_layered_state_machine", feasible
...
def _summary(rows: Sequence[Mapping[str, Any]]) -> dict[str, Any]:
    ...
    return {
        ...
        "independent_nav_reconciliation_difference": abs(
            standalone_nav - (float(rows[-1]["equity"]) / STATE_MACHINE_PROTOCOL.capital_usdt)
        ) if rows else 0.0,
    }
...
def _segments(rows: Sequence[Mapping[str, Any]]) -> list[dict[str, Any]]:
    return [
        ...
        for segment in STATE_MACHINE_PROTOCOL.segments
    ]


def build_state_machine_report(
    ...
    *,
    ...
    validate_state_machine_preregistration(preregistration)
    protocol = STATE_MACHINE_PROTOCOL
    ...
    candidate = run_variant(
        aligned, funding_by_symbol, rules,
        recovery_enabled=False, base_config=base_config, capital_usdt=protocol.capital_usdt,
        desired_weights_selector=StateMachineSelector(),
        gross_cap_policy="renormalize_active_targets_with_filter_floors",
    )
    ...
    report = {
        "schema_version": STATE_MACHINE_REPORT_VERSION,
        "artifact_type": "crypto_multi_speed_state_machine_trial",
        ...
        "diagnostics": {
            ...
            "verdict": (
                "retain_state_machine_for_family_review"
                if retained
                else "reject_state_machine_family_review_required"
            ),
            "candidate_pnl_ready": False,
            "promotion_evidence": False,
            "orders_authorized": False,
            "remaining_blockers": [
                ...
                "family_review_required_after_third_trial",
            ],
        },
    }
    trajectories = {
