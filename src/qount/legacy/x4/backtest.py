...
Feeds one bar stream to a :class:`~qount.legacy.x4.strategies.Strategy`, sizes its signed target weight
**equity-normalized** (``target_base = weight × equity / close``), trades the unified
:class:`~qount.legacy.x4.account.X4Account`, optionally settles funding, and snapshots equity each bar.
...
from qount.research_data import metrics as _stats
from qount.legacy.x4.account import X4Account
from qount.research_data.indicators import ATR
from qount.legacy.x4.strategies import GridStrategy
from qount.legacy.x4.strategies import PairStrategy
from qount.legacy.x4.strategies import Strategy
...
def run_kalman_pair(
    ...
    *,
    ...
    latch) but the z-signal comes from :class:`~qount.research_data.indicators.KalmanHedge` — β adapts online so
    ...
    from qount.legacy.x4.strategies import KalmanPair
...
def run_cross_sectional(
    ...
    *,
    ...
    from qount.legacy.x4.strategies import cross_sectional_rank
