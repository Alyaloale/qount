...
class EnsembleStrategy:
    ...
    stays in ``[-1, 1]`` so it drops straight into :func:`~qount.legacy.x4.backtest.run_directional` (and its
...
class VelocityScalper:
    ...
    live in the driver :func:`~qount.legacy.x4.backtest.run_scalper`, which can fill them intrabar.
...
class GridStrategy:
    ...
    in the driver :func:`~qount.legacy.x4.backtest.run_grid`, which reuses ``qount.legacy.grid_b.engine.GridLadder``
...
class PairStrategy:
    ...
    its logic lives in the driver :func:`~qount.legacy.x4.backtest.run_pair`. ``z > +entry_z`` (ETH rich)
...
class KalmanPair:
    ...
    Replaces the static rolling z-score with :class:`~qount.research_data.indicators.KalmanHedge`: β (the hedge
    ratio) adapts online, so the signal is the standardized Kalman forecast error rather than an
    OLS-window z that lags a regime shift. Driver: :func:`~qount.legacy.x4.backtest.run_kalman_pair`. Same
