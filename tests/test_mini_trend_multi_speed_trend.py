...
class MultiSpeedTrendTests(unittest.TestCase):
    def test_preregistration_is_result_free_and_hash_bound(self) -> None:
        payload = build_multi_speed_preregistration("2026-07-25T00:00:00+00:00")
        validate_multi_speed_preregistration(payload)
        self.assertFalse(payload["meta"]["strategy_results_evaluated"])
        self.assertEqual(
            payload["protocol"]["protocol_hash"],
            MULTI_SPEED_TREND_PROTOCOL.protocol_hash,
        )
        ...
        with self.assertRaisesRegex(ValueError, "contract hash mismatch"):
            validate_multi_speed_preregistration(tampered)

    def test_one_bar_delay_does_not_read_the_latest_signal_bar(self) -> None:
        bars = _bars(240)
        selector = DirectionConsistencySelector(signal_delay_bars=1)
    ...
    def test_direction_consistency_weights_are_long_only_and_gross_bounded(self) -> None:
        weights, mode, feasible = DirectionConsistencySelector()(
            _bars(240), research_rules(), 400.0, frozen_top3_config()
        )
        self.assertTrue(feasible)
        self.assertEqual(mode, "multi_speed_direction_consistency")
    ...
    def test_report_reconciles_nav_and_preserves_no_order_contract(self) -> None:
        preregistration = build_multi_speed_preregistration("2026-07-25T00:00:00+00:00")
