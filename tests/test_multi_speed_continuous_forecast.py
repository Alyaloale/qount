...
class ContinuousForecastTests(unittest.TestCase):
    def test_preregistration_is_result_free_and_hash_bound(self) -> None:
        payload = build_continuous_forecast_preregistration("2026-07-25T00:00:00+00:00")
        validate_continuous_forecast_preregistration(payload)
        self.assertFalse(payload["meta"]["strategy_results_evaluated"])
        self.assertEqual(
            payload["protocol"]["protocol_hash"],
            CONTINUOUS_FORECAST_PROTOCOL.protocol_hash,
        )
        self.assertEqual(CONTINUOUS_FORECAST_PROTOCOL.global_trial_number, 146)
        self.assertEqual(CONTINUOUS_FORECAST_PROTOCOL.trial_number_within_family, 2)

        tampered = copy.deepcopy(payload)
        tampered["decision_contract"]["signal"]["direction_lookbacks"] = [10, 20, 30]
        with self.assertRaisesRegex(ValueError, "contract hash mismatch"):
            validate_continuous_forecast_preregistration(tampered)

    def test_one_bar_delay_does_not_read_the_latest_signal_bar(self) -> None:
        bars = _bars(420)
        selector = ContinuousForecastSelector(signal_delay_bars=1)
    ...
    def test_continuous_forecast_weights_are_long_only_and_gross_bounded(self) -> None:
        weights, mode, feasible = ContinuousForecastSelector()(
            _bars(420), research_rules(), 400.0, frozen_top3_config()
        )
        self.assertTrue(feasible)
        self.assertEqual(mode, "multi_speed_continuous_forecast")
    ...
    def test_report_reconciles_nav_and_preserves_no_order_contract(self) -> None:
        preregistration = build_continuous_forecast_preregistration("2026-07-25T00:00:00+00:00")
        ...
        self.assertFalse(report["diagnostics"]["gates"]["funding_history_complete"])
        ...
        self.assertTrue(
            report["diagnostics"]["verdict"].startswith("reject_continuous_forecast")
            or report["diagnostics"]["verdict"].startswith("retain_continuous_forecast")
        )
