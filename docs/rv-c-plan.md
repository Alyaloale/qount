...
> **状态**：frozen（RV-C research-only / carry disabled，原地保留）｜索引见 [archive/README.md](archive/README.md)｜当前事实以 [current.md](current.md) 为准。
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-06-19 | ✅ carry arm 成功(实盘 hedged)+ 看板 | 第二次 arm 暴露第 2 bug `-2019 margin insufficient`(round(7.5)=8 张=$80 需 $26.7>provisioned $25);spot-first 再次保住安全裸多。**手动补 7 张空 ETHUSD_260626 完成对冲 → carry LIVE+hedged**(现货$50多+COIN-M$25保证金+−7张@$1691/isolated 3x/liq$2510=+48.5%/净Δ+$4.6)。修第 3 bug:合约数 round→**floor**(空≤多/保证金永够),+1 测 rv 共 81 绿。**看板**:趋势曲线清$75划转假崖(rebuild from snapshots,clean ~$414)+ 趋势 inception 484.57→410.07(P&L 不显假亏=+$4.35)+ **C×D 面板接活数据**(read-only `cxd_publish.py`+cron 每5min,查 trend+carry 仓→`cxd_live.json`,面板亮「实盘」carry$75/Δ中性/total$485)。**🔴 时限**:ETHUSD_260626=6/26 到期近月(基差≈0)、roll_buffer 5天→**~6/21 须 roll 到 9月**;但 carry 交易 cron 未开(autofund 非幂等会重复买多腿,待修)→ **6/21 前修 autofund 幂等+开 cron 否则手动 roll**。当前=静态 hedged 仓+只读看板(安全,不自动 roll)。 |
| 2026-06-19 | 🔴 首次 arm 事故 + 执行硬化(dry 掩盖的 bug) | **ETH-only $200 首次实盘 arm 暴露 2 个 dry 测不到的执行 bug,已安全拆平 + 修复**。arm 后:① `place_carry_orders` 给 COIN-M dated 单传 notional/空 params,但**COIN-M inverse 单要整数张合约** → `-1102 Mandatory parameter 'quantity'` 崩;② autofund 的 `transfer_coin_to_cm` 用**买入前的预估币量**(市价买后扣费实到更少)→ "余额不足"、币留现货没进 COIN-M。结果:$75 USDT 划到现货 + 买了 $75 ETH 全留现货、**保证金没进 COIN-M、空腿没开 = 裸多 $75**(现货无杠杆无清算,安全)。**owner 选「立即拆平回安全」**:卖 0.0441 ETH→$74.82、划 $74.35 回 UMFUTURE,**回到 flat**(UMFUTURE 复原 $322、COIN-M 空、carry 未武装),round-trip 费 ~$0.7,趋势腿未动。**修 3 处 + 5 单测(rv 共 80 绿)**:(1) COIN-M 单按 `round(notional/CONTRACT_USD)` 整数张下;(2) 币划转改用**实际 free 余额**(轻 haircut);(3) **spot-first 排序**——部分失败的残留是**裸多(有界/无清算)而非裸空(杠杆/可清算)**,空腿失败非致命 + 大写 ALERT + 下轮自动重对冲。dry 复验:$200 → BUY spot $50 + SELL COIN-M ETHUSD ×8 张。**教训:dry 不调 create_order/transfer,两场所真实执行(合约张数/扣费/划转/排序)只有真金第一单暴露**(同趋势腿 -4130/algo 坑)。**未重 arm**——建议下轮**更小($50-100)+ 盯盘**再上(两场所非原子,需监控)。 |
| 2026-06-19 | delta-中性修复(arm 前) | **live carry 改成真 delta-中性(owner 选「先修再 arm」)**。发现 live 有 `+N/L` 净多偏移:COIN-M 是币本位,空腿要币当保证金,旧 `target_legs` 现货多腿=全 N + 保证金币 N/L → 总多 N+N/L vs 空 N = 净多 N/L(回测从未建模这块,它是 spot-N vs short-N 中性)。修:`target_legs` 现货多 = `N·(L−1)/L`,使 **现货多 + COIN-M 保证金币(N/L)= N 总多 = 空 N → 净Δ≈0**(保证金币算进对冲,live 现与回测 N-vs-N 模型一致)。`plan_funding` 同步(保证金从空腿 N/L 算、只买缺口、USDT 需求=现货多+保证金缺口)。**回测复核**:`rv_c_basis.py` BTC L=3 净 +33.32%/+5.67%yr/6/6 年/三 gate 全过 = 与 §7 一字不差(backtest.py 未改,修的是 live → edge 不可能破)。+delta 不变量单测 → **rv 共 75 绿**。部署 VPS(md5 一致)+ 真实数据验:cap$200 ETH 现货$50+保证金$25=多$75 vs 空$75、cap$1000 BTC/ETH 各 多$375 vs 空$375 → **净Δ $0.00**(旧为 +$125)。代价:留 `N/L`(L=3 时 25%)per-pair 资金闲置(中性的对价)。**未 arm**(owner:下一轮)。 |
| 2026-06-19 | 自动供资(owner「用合约闲置资金」) | **carry 加自动供资:从闲置 USDⓈ-M USDT 供资,免手动**。owner 选「保持 COIN-M + 建自动供资」。纯件 `rv/live.py:plan_funding`(每对:现货多腿需 N USDT、COIN-M 空腿需 N/L 币本位保证金 → 计划 USDT 划 UMFUTURE→SPOT + 现货买保证金币 + 币划 SPOT→COIN-M)+ `execute_funding`(dry/armed 门控,ccxt transfer+buy,缺权限非致命)。**安全**:`um_buffer_usdt=200` 默认留 UMFUTURE 不抽干趋势腿保证金;`autofund` 默认关(`QOUNT_RV_AUTOFUND=1` 开)。+6 单测→**rv 共 75 绿**。已部署 VPS(md5 一致)+ 对**真实余额**验:spot $0 / UMFUTURE 闲置 **$323** / COIN-M 空 → cap $200 计划抽 $100 供 ETH-only(transfer $100 + 买 $25 ETH + 划 0.0146 ETH 到 COIN-M),cap $300/$400 **正确被缓冲挡下跳过**(不抽干);runner dry 端到端跑通(无 key 时优雅跳过供资)。**🔴 唯一阻塞**:API key `permitsUniversalTransfer=False` → owner 须在币安 API 管理勾上 **「Permits Universal Transfer」**(非提币,自有钱包间转账)才能真划;在那之前 transfer 调用会失败(已 catch 非致命)。**现状能力**:闸 SHUT 中趋势腿占 ~$160 保证金、UMFUTURE 闲置 ~$323,留 $200 缓冲 → 现可自动 bootstrap ETH-only carry(~$200);BTC+ETH($300+)需更多 UMFUTURE 余量或调低缓冲。 |
| 2026-06-19 | 实盘腿 re-verify | **carry sleeve 实盘腿已建 + dry 跑通(VPS 复验)**。澄清:`src/qount/rv/live.py`(纯件:`active_dated`/`target_legs`/`compute_carry_orders` + roll)+ runner `scripts/desktop/rv_live.py`(双场所 spot+COIN-M)+ `test_rv_live.py` **早在 6/17 建好**(commits f79d80a/eab71e6/d79364c),非待建。本轮同步当前版到 VPS(md5 一致)+ 复跑:**rv 共 69 单测全绿**;VPS dry 端到端跑通——识别活跃季度 `BTCUSD_260626`/`ETHUSD_260626`,$1000 出 4 腿 delta-neutral(±$375/对,Δ$0)、$300 出 ±$112(均过合约下限),状态落 `state/rv/live/latest.json`。**唯一阻塞 = 注资 + 权限**(非代码):① 现货钱包 USDT(买多腿)② COIN-M 交割钱包存 BTC/ETH 币(空 inverse dated 的保证金)③ API key 加 **SPOT + COIN-M(delivery)** 权限(当前 key 仅 USDⓈ-M)。**最小注资**:per-pair N=cap/2×L/(L+1),pair 在 N<合约面值($100 BTCUSD/$10 ETHUSD)跳过 → **ETH-only 需 carry slice ≥ ~$27、BTC+ETH ≥ ~$267**(经 cxd 编排 carry=总×40% → BTC+ETH 需总 ≥ ~$667)。**arm**:`QOUNT_RV_LIVE_ENABLE=1` + 设 `QOUNT_RV_CAPITAL` + COIN-M key;cron 有注释好的 cxd 编排行待注资改回。**为何值得**:闸 SHUT ~50% 时间趋势腿在现金,carry 正是该空窗唯一正收益腿(§7.10 / C×D corr −0.196、+40% carry → Sharpe 0.89→1.18/maxDD 砍半)。 |
