...
> **2026-06-06 经验证伪（见 §11.8）**：本段的「有效独立下注 ~数百」高估了广度。实测 top12
> 平均绝对两两相关 r̄≈0.63，有效广度 `N/(1+(N-1)·r̄)` 仅 ≈1.5，且 N→∞ 渐近天花板 1/r̄≈1.6——
> 扩币救不了。真实要求 IC ≈0.15（非 0.06），回到「不可达」区间。横截面广度杠杆结构性不存在，
> 这是触发项目级 §7 止盈的架构级证据。
...
> **2026-06-06 更新：换特征源第一刀（funding 作预测特征）已证伪。** 新增 `xs_funding` /
> `xs_funding_rev` 族（funding as-of join 无前视当横截面信号，复用 IC/DSR/PBO harness），
> top12 120 天 discovery、post-cost 读数：rank-IC 全 ≤ `0.030`（< 价量动量 `0.052` < 要求
> `0.06`）、carry-tilt DSR ≈ `0.25`（best per-period sharpe `0.094` < 噪声期望最大 `0.156`）、
> PBO ≈ 0.49–0.55、唯一正 cell（`1d xs_funding_rev h6`）rank-IC ≈ 0 系噪声、4h/8h 全被成本
> 打负。继 CARRY 现金流（basis-tail 证伪）之后 **funding 这一源两种用法均证伪**。剩余 named
> 源（微结构无廉价历史盘口、时序基础模型需重 ML 栈/libomp）都不是廉价 kill-test，第一轮换源
> kill-test 倾向 §7 诚实止盈。证据链见 update-log.md 2026-06-06 与 current.md。

**不变的硬约束**：§6 / §10.7 全部继承；live 关闭、不 forward paper、不放宽 broad gate、
validation_v1 once-only、一轮只改一处。诚实退出不放宽任何一条，只是停止在已证伪的候选上投入。

### 11.8 架构级根因：横截面广度天花板，§10.2 破局数字被经验证伪（2026-06-06）

funding 换源 kill-test 之后，没有继续机械试第三种特征源，而是回头查 §10.2 整套破局逻辑的
**前提**——「日频横截面 ~10 币 → 有效独立下注 ~数百 → √BR≈17 → 要求 IC 0.06 可达」——是否成立。
直接读 funding artifact 已报的 `effective_breadth`（标准公式 `N/(1+(N-1)·r̄)`，r̄=平均绝对两两
收益相关）：

```text
top12 r̄ ≈ 0.628（4h 0.620 / 8h 0.635 / 1d 0.628）
有效广度: N=12→1.52  N=20→1.55  N=30→1.56  N=100→1.58  N=1e6→1.59
渐近天花板 1/r̄ ≈ 1.59      ← 扩币在数学上救不了（N→∞ 仍 < 1.6）
要求 IC(Grinold, IR=1):  §10.2 假设~10币 IC_req=0.058  vs  真实~1.5币 IC_req=0.148
观测最强横截面 IC:        xs_mom 0.052 / xs_funding 0.030   → 离真实要求 ~3x 缺口
```

**判定**：§10.2 是全文最大的单一杠杆假设，现被经验证伪。加密 majors 同涨同跌（r̄≈0.63），
横截面把「12 币」折成 **~1.6 个有效独立资产**，且这是 `1/r̄` 渐近天花板——**扩币、换特征源都
无法突破**。真实要求 IC ≈ 0.15，回到 §10 想逃离的「5m 单币不可达」区间。这统一解释了为什么
xs_mom（0.052）、ts_mom、xs_funding（≤0.030）全部过不了线：**根因是广度，不是特征或频段**，
所以继续在特征/频段空间里搜索只会触发 §7 警示的「搜了 N 个 cell 的多重检验假象」。

**结论**：本项目在「latency-insensitive、retail-accessible、低换手」的约束下，可触及的
盈利路径——横截面（XS-MOM/REV/funding，广度天花板封死）、日频时序（TS-MOM，§11.4 已证伪）、
CARRY 现金流（basis-tail 已证伪）——均已穷尽且证伪。**这满足 §7 的全局终止条件**（§10.7 改版：
「S1' 所有频段 × 族在 holdout 上都无 post-cost 正 edge」——现已叠加「且广度杠杆结构性不存在」的
更强证据）。

**决策（2026-06-06，项目所有者确认执行）：执行 §7 末段的项目级诚实止盈。** 承认当前数据 /
成本 / universe 相关结构下不足以支撑择时盈利；把研究价值（一整套反过拟合 harness：triple-barrier、
meta-label 思路、purged-CV + embargo、Deflated Sharpe、PBO/CSCV、effective-breadth 诊断、
频段 × 族选择扫描）作为成果固化，**停止在择时盈利上继续投入**。后续若要重启，触发条件应是
**结构性新输入**（真正低相关的新 universe / 新资产类别、或可执行的低延迟微结构通道），而非在
已穷尽的特征 / 频段空间里继续搜索（那只会触发 §7 的多重检验假象警示）。
