...
> **状态**：blueprint/guarded（重构蓝图，已归档）｜索引见 [archive/README.md](../../README.md)｜当前事实以 [current.md](../../../current.md) 为准。
...
**OOS 验证(2026-06-09 续):反向 carry = 符号 OOS 成立但量级 sub-gate,不晋级。** 所有者选"做真 OOS"。事前钉死:(a) 经济理由=中国商品零售/投机主导,陡 backwardation=近月拥挤多头→均值回归跑输→**事前承诺反向符号**(拥挤反转);(b) 独立窗口=从未碰过的 max-history..2022-05-31(免费天勤回到 ~2020,600 bars,与样本内 2022-06+ 不相交);(c) OI 加权流动主力重构留作符号活下来后的精修。给 `fetch_tqsdk_carry_panel` 加 `end_date`(纯构建函数不动,86 测过)。**OOS 实跑:① 原始 carry→fwd21d 截面 rank-IC=−0.0147(符号仍负,反向方向 OOS 成立——不是 2022-26 纯巧合),但量级是样本内 −0.038 的 ~40%;② 反向 carry 门控 BELOW(best 0.85、DSR 0.593<0.95、PBO 0.33✓、folds 0.80✓);③ selection-free walk-forward NOT ROBUST(ensemble 0.48<0.5 / wf 0.64 / fixed 0.90)。** 判词:**符号方向真实且 OOS 持续,但样本内 1.17 是 regime 灌高,OOS 只剩弱 tilt、不过门控**——撞回 §8/L1 同一堵墙:**信号真、零售单 sleeve 量级不够(趋势~0.4、反向 carry OOS~0.48,皆<0.5)**。按纪律 sub-gate 不晋级。**按年 IC 诊断(回应"2020-22 疫情压低"质疑):证伪该质疑,且把判决从"sub-gate"收紧到"非 edge"。** carry→fwd21d 截面 rank-IC 按年:2020 +0.008 / 2021 −0.079 / 2022 −0.008 / **2023 −0.248** / 2024 +0.065 / 2025 +0.022 / 2026 +0.108。**反向 carry 本质是 2023 单年现象**(一年扛起样本内 1.17),弱**不在**疫情年(2020/2022 本就 ~0、2021 反向还行);**致命的是 2024/25/26 符号翻回正**(近年反向 carry 反而亏、经典方向轻微回归)。**符号逐年翻 = 无稳定溢价**,本项目反复证伪的"单窗口扛全部、出窗口符号不稳"同一指纹。固化:tqsdk 数据层 + carry sleeve + 期限结构 harness 作可复用资产;国内商品 carry(任一方向)非 edge,单 sleeve 追盈利按 §7/§8 同型止盈。下一次抬量级仍需结构性新输入(多 sleeve 风险平价需各 sleeve 先独立过线 / 真期货组合规模 / 付费天勤拿 2010-2019 干净 pre-COVID 历史做真长期 OOS)。artifact `state/research_runs/manual-cta-tqsdk-carry-oos.json`(WSL)。

### 11.11 跨资产境内期货趋势 = 从反方向钉死"广度非约束";免费期货线诚实止步(2026-06-09)

回应"没付费是否止步":商品单一品类注定受限,真 CTA 在跨资产+做空+利率腿。测 22 标的(16 商品 + 股指 IF/IC/IH + 国债 T/TF/TS,long/short,免费天勤数据全有,交易股指/国债需 ¥50万验资但研究不需要)。**实跑(同 2020-2026 窗口 1575 bars):有效广度 3.97(全项目最高)、但门控 best Sharpe 0.43(< 商品单测 0.82)、walk-forward ens/wf/fixed=−0.10/−0.19/0.07(~0,比商品单测 0.25/0.40/0.47 还差)、BELOW GATE。** 根因:2020-26 是中国权益趋势旱季(A股 2021-24 长期震荡),股指 trend 在震荡亏钱,把组合从商品 0.82 拖到 ~0(§11.2 的 long/short 指数 1.05 是 2015 灌的,不在免费窗口)。

**这把刀的价值=从反方向钉死结论:广度做到 3.97 仍 ~0 收益 → 广度彻底不是绑定约束(从来不是),绑定约束是 2020-26 这个窗口的原始趋势 edge 量级 ~0.4 封顶,加再多震荡资产只稀释。** 与全局 §7/§8 同源(天花板=edge 量级,非广度/long-only/单品类)。

**免费境内期货线诚实止步:** 四角度验证一致(商品趋势 0.4 / 跨资产趋势 ~0 / 经典 carry 证伪 / 反向 carry 2023 假象),全 sub-gate。**不再"换信号搜"(MR sleeve 等)= §7 跑步机。** 重开仅靠结构性新输入:① 付费天勤 pre-2020(2010-19 强商品趋势+2015,测是否只是 2020-26 旱季——买的是验证已知非新发现);② 等趋势 regime 回来(§8 部署后熬旱季);③ 接受 §8 现实(好年 0.5-0.8/旱季走平)。固化:tqsdk 数据层 + carry sleeve + 跨资产期货 universe + 期限结构 harness。**部署中的 ¥100万 ETF 主线不受影响,仍是唯一过线在跑的可交易主线。** artifact `state/research_runs/manual-cta-tqsdk-crossasset-trend.json`(WSL)。
