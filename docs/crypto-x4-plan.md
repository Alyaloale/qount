...
> **状态**：legacy / live disabled（线 D，原地保留）｜索引见 [archive/README.md](archive/README.md)｜当前事实以 [current.md](current.md) 为准。
...
## 0.1 2026-07-17 日线 headline funding 更正（历史 artifact 不覆盖）

旧日线回测的 funding callable 使用 `funding_ts == daily_bar_open_ts` 精确匹配。Binance USD-M 每天结算
3 次，因此该口径只命中 00:00，漏掉 08:00/16:00。新的离线归因 artifact
`state/research_runs/20260717T121616Z-mini-trend-high-return-attribution/mini_trend_high_return_attribution.json`
保留并逐位复现旧 artifact 后，再按完成日线收盘后的下一持有日汇总三次 settlement：

| 策略 | 旧 headline | 完整 funding | 虚高 | Sharpe 更正 | maxDD 更正 |
| --- | ---: | ---: | ---: | ---: | ---: |
| S3-CTA | +115.18% | **+84.93%** | 30.25pp | 0.665→0.559 | 29.43%→31.50% |
| S4-MOM | +98.39% | **+70.77%** | 27.62pp | 0.698→0.570 | 23.78%→24.81% |

收益来源桥接仍成立，但读法更收敛：1h→1d 为 S3/S4 增加 `+61.46/+57.94pp`，去掉空头增加
`+59.42/+63.40pp`，SMA200 闸增加 `+48.56/+58.28pp`；波动率定仓和 S4 吊灯改善风险形态。
2x cap 相对 1x 只贡献 `-1.17/+2.54pp` 且 Sharpe 都下降，故旧高收益不是杠杆 edge。同期正确 funding
的 BTC 1x/TOP3 EW 是 `+37.72%/+355.40%`、maxDD `78.93%/73.59%`：趋势书本质是低 beta 牛市择时，
不是已证明的稳定 alpha。旧表作为历史实验记录保留，但后续任何判断必须引用本更正口径；
`x4_compare.py` 已改用公共 `holding_period_funding` 汇总下一持有期全部 settlement，新 artifact 会显式写
`funding_alignment`，不再默认生成旧错误口径。修后终表是
`state/x4/research_runs/x4_compare_2021-01_2026-05_1d_20260717T122845Z.json`。
...
- **迁 VPS(墙外 qount-vps)**:全部部署从 Mac launchd 搬到 VPS crontab(Mac 是 git 表面、改完 scp;VPS 非 git
...
| 日期 | 版本 | 变更 |
|---|---|---|
| 2026-07-11 | ops(VPS 资源耗尽事故修复) | **无锁 cron + 无总 timeout 导致的生产崩溃已修复并部署。** 2026-07-10 Binance/DNS 抖动触发 CCXT `load_markets()` 私有 `fetchCurrencies` SAPI 调用卡住；`*/2` live 与其他 cron 无防重入，旧进程不退、新进程叠加，1.6G VPS 最终 fork/内存饥饿，SSH/Caddy/new-api 全部协议层超时。新增共用 `cron_guard.sh`，active live/publisher/paper 脚本内层 non-blocking `flock` + process-group timeout，crontab 外层再兜底；live 110s、publisher 90s、paper 1800s。live `*/2 -> */5`，publisher 改 `2-57/5` 错峰；日线信号 + exchange-native STOP_MARKET 使该降频不改变策略/止损语义，同时减少 60% 调度/API 压力。C×D 删除编排层重复 private balance read；全局 Binance client `fetchCurrencies=False`，不再为 market metadata 访问 `/sapi/v1/capital/config/getall`；交易腿失败不发布伪新快照。**策略权重、2x、short gate、scale-out、carry paused 全不动**，没有 OOS 证据不借事故临时调参。local `976 OK`，VPS focused `141 OK` / full `865 OK`；旧 crontab 已备份。 |
| 2026-06-21 | ops(复盘后优化:账目修 + 旋钮 + 重定基线) | **基于实盘复盘的三处优化(owner 定 P1=B 重定基线,不充值)**。复盘发现合并账户自上线 −0.37%(基本走平),但看板**分腿盈亏误导**:`trend_pnl +$68(+22%)` 与 `carry +$4.4` 同时存在却与 total −$1.79 对不上。**① P0 分腿盈亏 flow-adjusted(核心)**:根因=各腿用静态 inception 锚,而 autofund 在趋势钱包↔carry 间搬钱 → carry 注资被误记为「利润」、趋势流出被误记为「亏损」。修法=新增纯函数 `x4/live.carry_flow_adjusted_pnl`(carry 盈亏 = 权益 − 成本基线 − 累计 autofund 注资,按 rv 快照 ts 去重防跨-cron 重复计),`trend_pnl = total_pnl − carry_pnl`(残差,恒对账)。`total_pnl` 仍是唯一硬真值。**+7 单测 → x4 共 98 绿/全套 889 绿**。VPS 验证:`trend_pnl` 从假 +$68 → 诚实 **−$6.1(−1.6%)**、carry +$4.45、`trend+carry = −1.65 = total` ✓。**② P1-B 重定基线(owner 定)**:carry 卡在 21%(ETH-only,BTC 不可注资 + COIN-M 整张颗粒度封顶),≠ 验证过的 40%。不充值 → 看板诚实标注「运营基线 ETH-only(设计 40% 需充值 ~$650 解锁)」+ 警示「真实尾部保护弱于 §20 的 maxDD −10%」,去掉误导性 60/40 目标显示(`web/site/app.js`)。**③ P2 做空敞口旋钮**:`QOUNT_X4_SHORT_VOL_TARGET` env 外置(0.03≈gross 0.6 / 0.02≈0.4),= owner 风险偏好旋钮(收益换波动),NOT 新 alpha 假设——震荡失血就调小,**不加择时 if 规则**(docs 已证伪 ADX/regime 增量)。**P0.5 信号滞后(bar 比已收盘日慢 ~1 天,因信号读 binance.vision 日包与回测同源、发布滞后)= 决定不修**:补 ccxt 实时 K 会给信号路径加 7×/2min 网络调用 = 重新引入刚修掉的超时脆弱性,而慢 regime 里 gate 不变、收益≈0 → 定性为已知可接受特征。 |
| 2026-06-21 | ops(carry 腿误报告警修) | **修 C×D carry 腿瞬时网络超时被当「实盘失败」误报告警**。现象:今天 11:50–12:12 CST,`cxd_live_cron.sh` 每 2 分钟推一条「C×D实盘告警·rv_live.py 失败」(Server酱 微信),共 7 条。根因:`rv_live.py:_build_coinm` 建的 COIN-M(`dapi.binance.com`)客户端**没设 `timeout`,吃 ccxt 默认 10s**;该端点当时网络抖动 → `cm_ex.load_markets()` 10s 读超时 → 整条 carry 腿 `rc=1` → cron 推告警。trend 腿(`api.binance.com`,且 cron 有 preflight ping)全程正常;carry 持仓本就 Δ≈0、无杠杆无清算,跳格只是当格不重对冲(代码注释:「现货多腿可能裸露(安全),下轮自动重对冲」)→ **无仓位损失,纯误报**;12:14 起自愈。**修复**(`scripts/desktop/rv_live.py`):① `_build_coinm` 加 `timeout=30000`;② 新增 `_load_markets_resilient`——对 `load_markets()` 做最多 3 次退避(2s/4s)重试,**只吃 `ccxt.NetworkError`**(超时/限流/不可用),`AuthenticationError` 等真错误仍立即抛出(`issubclass` 验证:`RequestTimeout`⊂`NetworkError`、`AuthenticationError`⊄`NetworkError`);spot+COIN-M 两个 `load_markets` 都走此封装。**验证**:scp 到 VPS dry-run 恰好撞上一次真实瞬时超时 → 日志 `(load_markets transient RequestTimeout, retry 1/2 in 2s)` 后正常跑完落盘(旧码此刻就会 `rc=1` 推告警)。教训:delta-neutral/无清算的 carry 腿,瞬时网络抖动不该当作实盘失败 page。 |
| 2026-06-19 | 研究(继续找方向 = 全负/再确认天花板) | owner「接着找别的方向」。**universe 多样化(`x4_universe_wf.py`,walk-forward)= 证伪**:TOP7→TOP10/ALL18 全线更差(ALL18 +101%/0.81/test 0.57,broader OOS Sharpe 0.74 vs TOP7 1.01,≥TOP7 仅 2/9 折)——低档山寨趋势更噪+崩盘一起 dump(尾不分散/maxDD 更深),「分散>选择」有质量上限,TOP7 已是好选择。**加权(inverse_vol_corr 1.07 > inverse_vol 1.05 > equal 0.79)+ 闸口径(OR 远胜 AND 0.53/breadth 0.67)= 现状再确认最优**。**元结论:趋势书本体到顶,继续扫参数低 EV;唯一未吃掉的大块上行 = 闲置现金结构性低效(大盘闸 SHUT ~50% 时间本金在现金赚 ~0)→ C×D delta-neutral 资金费 carry sleeve(线 C,§20 corr −0.196/+carry Sharpe 0.89→1.18/maxDD 砍半),是 spot+COIN-M 注资 + 建 `rv/live.py` 的部署决策,非趋势参数调优。** 详见 `x4-live-position-management.md` §12.2-12.3。 |
| 2026-06-19 | 研究(轻量 ADX walk-forward = 证伪) | **§12 看好的轻量 ADX 门经 walk-forward 拍掉,不上 live**。`x4_adx_walkforward.py`:9 滚动 OOS 折(train504/test126/step126)。固定 adx14 OOS 均值 Sharpe **0.99 < 基线 adx0 1.01**(略劣)、maxDD −8.1% vs −8.5%(几乎平),Sharpe≥基线仅 5/9 折、**maxDD 不更深仅 4/9 折**(多数折 ADX 反把回撤做更深);§12 全样本 −4pp maxDD 几乎全由单一事件(2024-02..06 折 adx14 −11.3% vs base −16.5%)贡献,OOS 不复现。真 WF(train 选 adx→OOS)**反更差 0.89<1.01** 且选择不稳({0,14,16} 各 1/3)。**= §12「ADX 增量」是全样本单 regime 假象,原「ADX 非 alpha」成立**。**教训:全样本 train/test 双段过 ≠ 滚动 OOS 过,maxDD 类改善尤其易被单期主导——上真金前必过 walk-forward**(本次正是它把一个会上真金的过拟合挡在门外)。趋势/杠杆/刷新侧无可上线增量,baseline 即天花板再确认;已上线真增量止于分批止盈+做空闸+收益口径修。详见 `x4-live-position-management.md` §12.1。 |
| 2026-06-19 | 研究(四方向深度优化)+ live(-4130 修) | **① 修 `-4130`/`OrderNotFound` 止损 amend churn**:根因=Binance closePosition STOP_MARKET 是 CONDITIONAL/algo 单(algoId,非常规单),`cancel_order` 默认打常规端点→OrderNotFound→旧 algo 止损残留→re-place -4130 每轮 churn(trail 卡死不收紧)。修=`cancel_order` 加 `params={"stop":True}`(路由 algo 取消端点)。验:post-deploy cron 00:20 = -4130 x0/cancelfail x0(前序 old-code 每轮 x6),止损更新到新 trail。+mock 断言。**② owner 点名四方向深度分析**(`x4_optim_analysis.py`+`x4_refresh_freq.py`,基线对账 §24.1 +172.8%/1.07/−26.1%):**(动态杠杆)无 alpha**——Sharpe 跨 vt0.02-0.04 恒定 1.07、max_lev cap 惰性(1.5≡2≡3)、T3-8 风控旋钮;杠杆=纯风险偏好选择。**(仓位管理)** band 0.4 轻微改善(噪声偏大,非干净平台);分批止盈(§10 已上)才是主胜利。**(趋势判断)** 核心参数复核全在最优(slow60 唯一 train≥1.0、regime200、breadth0.5)=天花板确认;**🟢 新发现=轻量 ADX(adx_min 12-16 平台,非单点):adx14-16 总收益持平+Sharpe +0.03+maxDD −26→−21.5~22.7%+train/test 双段不劣,修正旧「ADX≥20 非 alpha」(过紧);ADX15+band0.4 叠加=+183%/1.11/−21.9%/tr1.00/te1.19** = 风险质量增量(非纯 alpha,过双段),**唯一值得推进的趋势候选,建议 walk-forward 验稳后 env-arm 上 live**。**(刷新频率)** 日线 vs 4h:4h 总收益虚高但 maxDD 翻倍 −25→−56%(2x=清算区)+OOS test Sharpe 1.24→0.99+train1.43≫test=过拟合 → **日线信号正确,确认 §11「日内=噪声」**;cron 已 2min 实时反应止损,只趋势信号留日线=对。详见 `x4-live-position-management.md` §12。 |
| 2026-06-19 | live(收益口径修 + 做空 0.6) | **① 修收益虚高 ~2×(未实现重复计)**:owner 发现「总盈亏≈真实两倍/没平仓哪来已实现」。坐实根因——ccxt `fetch_balance['USDT']['total']` 在 USDⓈ-M = **marginBalance(含未实现)**,runner 当 `capital` 后又 `equity=capital+未实现` → 未实现加两遍(实测 wallet 484.91/uPnL 4.06/margin 488.97=total)。修:纯件 `live.py:usdt_wallet_balance` 取 walletBalance(优先 `info.totalWalletBalance`),`capital`=walletBalance、`equity=capital+未实现=marginBalance`(只一遍）。VPS 验:484.90+3.51=488.41=margin✓、total_pnl 488.41−484.57=$3.84(真实非~$7)。sizing 顺带转 walletBalance(=§9 验过~无差且更对)。+4 单测。**② 做空 gross 0.40→0.60(owner 指定)**:正确旋钮=`short_vol_target`(cap 1.5 惰性,vt0.03 下 lev1.5≡2.0 不 bind);`short_vol_target 0.02→0.03`。回测拼账 +246.5→**+311.6%**/Sharpe 1.17→**1.23**/train 1.04→1.12+test 1.27→1.33 双升/maxDD −30.2→−31.1%(~1pp 深)。副益:0.6 下 BTC 空目标>最小 lot→不再跳过,消除山寨集中偏斜。已部署(默认改,reconcile 自动加空到~0.6,实测 5 单)。⚠️ 偏离 §24 vt0.02 中心=owner 知情收益旋钮加注。x4 共 **867 绿**。详见 `x4-live-position-management.md` §11。 |
| 2026-06-18 | live(分批止盈接入 + 部署) | **§10 分批止盈接进实盘 + 部署 VPS + armed**。纯件 `live.py:apply_scale_out`(同向持仓按 `entryPrice` 算浮盈,每 +step 永久砍 frac、floor residual、ratchet 不回吐加仓,状态 `scale_out.json` 跨 cron;多空对称)+ runner 在 `apply_chandelier_stops` 后接入 + 快照加 `scale_out/scaled_out/scale_out_steps`;`LiveConfig.scale_out_*` 默认 0(关时字节级一致),arm 开关 `QOUNT_X4_SCALE_OUT=1`→推荐档 step0.20/frac0.50/residual0.34。+6 live 单测→**x4 共 863 绿**。**部署**:scp 三件(live/backtest/account)+ runner 到 VPS,md5 校验一致;env 加 `QOUNT_X4_SCALE_OUT=1`(已备份);dry+live 端到端跑通。**当前 inert**(6 空仓浮盈 <20%→steps 全 0、0 减仓单,待 +20% 自动落袋)。**最大仓位**:gross 硬顶 = `max_leverage×capital` = 2×$488 = **$977 名义 = 100% 买力**(vol-parity 正常只到 ~0.5-1x);当前空书意图 gross 0.40($193/19.8% 买力)、实持 ~$137(BTC 空 $50<最小 lot 跳过);**多头书当前为空(全币在 200MA 下=熊,无币可多)**。分批止盈只下调持仓不抬高开仓上限。**另注**:`live.py` 交易所兜底止损 amend 的 `-4130`/`OrderNotFound` churn 预存在复现(非本次引入、非致命,旧止损仍护仓),独立待修。详见 `x4-live-position-management.md` §10.1。 |
| 2026-06-18 | 研究(实盘仓位管理 §8/出场) | **出场/锁利回测:浮盈条件收紧=无 edge,分批止盈=真·风险调整改善(未接 live)**。owner 痛点「利润到手了又没了」:多头出场二元(信号掉→全平 / chandelier 宽 8×ATR ≈峰值回撤 28% 才走),无分批止盈/浮盈锁利。`run_directional` 加两组默认关旋钮(关时字节级一致):① `profit_lock_threshold`/`profit_lock_mult`(入场仍宽 8×,浮盈≥阈值切紧 Y×ATR);② `scale_out_step`/`scale_out_frac`/`scale_out_residual`(浮盈每 +step 永久减 frac、留残仓 trail、ratchet 不回吐加仓)。+4 单测→**x4 共 857 绿**。脚本 `x4_profit_taking.py` 复用 `x4_short_gate` 原语,基线对账 §24(+142.0%/0.91/−28.2%,train0.78/test1.01)。**结果**:**① 收紧=无 edge**(紧档毁收益、宽档总收益持平但 maxDD −30~32% 变差、train 普降);**② 分批止盈=同族唯一过双段**:每+20%减 33%/50%+留 1/3 残仓 → **Sharpe 0.91→0.99/1.00、maxDD −28.2%→−22.0%**,**train/test 双段都不劣**(减50%档 train 0.79≥0.78、test 1.22≫1.01=非过拟合 OOS 反更好),代价总收益 −18pp(+142→+124%)。**性质=风控/锁利旋钮(改善 Sharpe+DD 换总收益)非纯 alpha,但与 T3-8(FAIL 双段)不同=过双段且对症**;在 2x 永续上砍 maxDD/降回吐比现货回测更值。**推荐档 step0.20/frac0.50/residual0.34**。**未接 live**:`live.py` 走 `target_weights`+`apply_chandelier_stops`(非 `run_directional`),部署需在 live 路径补「浮盈分批减仓」(独立改动+单测),上线 owner 定。详见 `x4-live-position-management.md` §10。 |
| 2026-06-18 | 研究(实盘仓位管理 §8.3①) | **做空侧本金口径 walletBalance vs marginBalance = 不部署(机制真、量级二阶可忽略)**。`docs/x4-live-position-management.md` §8 解剖当前实盘(本金 auto=marginBalance,做空闸 6 仓 gross 0.29 = 买力 ~14%)发现:auto 本金含浮盈 → 空头赚钱时目标名义涨 → 对赢钱空头在探底区加仓(逆周期);候选修复=做空 sizing 改 `walletBalance`(已实现口径,不含浮盈)。**工程**:`X4Account.wallet_balance()`(=`equity` 减开仓 MTM)+ `run_directional(wallet_sizing=)` 默认关旋钮(关时字节级不变);研究脚本 `x4_short_wallet_sizing.py` 复用 `x4_short_gate` 原语,唯一变量=空头 sleeve 本金口径(多头书两臂共模),基线逐位对账 §24。+6 单测(account 3+volsizing 3)→ **x4 共 252 全绿**。**结果(配置=§24 中心 regime100/3x/2%·1.5x)**:wallet vs margin 几乎逐位重合——total +243.4% vs **+245.8%**(−2.4pp 略劣)、Sharpe 1.16=1.16、maxDD −30.2%=−30.2%、train/test 互有 ±0.02 微动、空头成交 477 vs 479(少 2 笔)。**根因**:做空 size 仅 2%/1.5x(gross ~0.3x)+ 紧 3×ATR chandelier 很快 latch 掉赢家,浮盈来不及喂回仓位、band 0.25 又吸收漂移 → "探底加仓"机制真存在但量级二阶。**归「已测·非增量」,旋钮默认关研究用,不接 live。** caveat:wallet 对称——同时拿掉浮亏保护性减仓。详见 `x4-live-position-management.md` §9。 |
...
| 2026-06-17 | feat(外部 review T2-2) | **交易所原生兜底止损建成 + 已部署 VPS 验真**。补 reviewer 指出的真漏洞=本地 chandelier 每 10min 一查,插针在两次 cron 之间可能先被交易所强平/穿过止损而 bot 还没发单。修复=持仓腿在交易所**挂一张 reduce-only `STOP_MARKET`(`closePosition=True`)兜底单**,停在 chandelier 价(`trail_high−mult×ATR`),trail 上移超 `stop_amend_band`(默认 1%)才 cancel+replace、band 内不动(幂等无 churn),不再持有/latch/闸关的腿撤掉残留挂单;`closePosition` 触发即整腿平、平仓后交易所自动撤。纯件 `live.py`:`chandelier_stop_prices`/`plan_stop_orders`(+`StopOrder`/`StopCancel`/`StopOrderPlan`)+ 薄 ccxt `fetch_open_stops`/`sync_stop_orders`(dry/`QOUNT_X4_LIVE_ENABLE` 门同 `place_orders`,closePosition 不需 amount)。LiveConfig 加 `exchange_stops=True`/`stop_amend_band=0.01`(swap only,False 则拆除)。runner 在 `place_orders` 后同步、快照加 `resting_stops`。**+16 单测 → x4 test_x4_live 63 全绿;grid106/rv69 回归绿**;dry 冒烟端到端跑通(闸关→0 止损符合预期)。**已 rsync+scp 部署 VPS + 真账户只读验真 `fetch_open_orders`/`fetch_open_stops` 签名(空账户返 {} 无错)+ armed 全路径 cron 端到端跑通(闸关→0 单、resting_stops={})**。剩余唯一未验=`create_order(STOP_MARKET, closePosition)` 实际挂单(空仓没法安全测,首次建仓即闸开时验真,单测已覆盖逻辑)。 |
| 2026-06-17 | 复核(外部 review T1) | **永续 funding 拖累复核 = 头条厘清两口径,reviewer "funding 吃 10-20%/yr" 不成立**。owner 转述一份外部 review(趋势质量过滤/动态 vt/资金费侵蚀/闪崩止损/集中度等),分诊后多数已被 §21.4(ADX=风控档非 alpha)/§11.2-11.3(做空全灾难)/线 C §7.9(基差条件化 carry 已证伪)覆盖。最尖锐一条=「头条 Sharpe 没扣永续资金费」→ 跑 `x4_live_strategy_backtest.py` 复核:**funding 其实早已建模**(commit 97b5fd1,`load_funding`+`run_trend_portfolio(funding_by_sym=)`+引擎 `accrue_funding`),旧头条 **+164.9%/0.99/−27.2% 是关 funding 的口径**;扣永续 funding 的**诚实数=+142.0%/0.91/−28.2%**(LIVE 全配置 2021-01..2026-06,1988 bars;train0.78/test1.01 无 overfit;chandelier 8× 几乎不 bind=灾难档如设计)。**funding 真实拖累仅 −22.9pp/5.5yr ≈ ~2pp/yr / Sharpe −0.08**——远小于直觉,因 **vol-parity 敞口 ~0.5x × 总闸半数时间空仓(gate%=50%)**,BTC/ETH 表面 ~11%/yr 费率乘下来才 ~2.7%/yr。**决策:reviewer 的「资金费极正停开新仓」过滤器抢救上限太薄,不做**;其余真增量按优先级排=① carry 腿微量注资跑通(空窗期正收益,§20,最高利润杠杆)② 交易所原生 reduce-only STOP_MARKET 兜底 10min cron gap ③ 待测档(MA200 斜率确认/动态 vt/周级再平衡/平仓冷却,各走 §21.4 train/test 门)④ 缓做(本金提到 $500-600 全覆盖 7 币/put 险/单币熔断)。诚实前向锚从此用 **+142%/0.91** 口径,不用 +164.9。 |
| 2026-06-19 | ops(§21.7 C×D实盘) | **C×D 组合实盘上线 + 三处 bug 修复**。承 §21.6 趋势 pilot 和 §20 C×D paper，部署 85%趋势(USDⓈ-M 7币做空)/15%carry(ETH 现货多+COIN-M 季度约空)组合 cron 体系。**三处 bug 根因与修复**:① `rv_live.py` 用 `fetch_positions()` 只返永续不返 COIN-M 交割合约 → 改 `dapiPrivateGetPositionRisk()`;② `CONTRACT_USD` 未 import(NameError 被 `except` 吞掉 → fallback `fetch_positions` 同样读不到 → 整个 COIN-M 读为空) → 加 `CONTRACT_USD` 到 import;③ `sym.replace("USD_","USD",1)` 把 `ETHUSD_260626` 变成 `ETHUSD260626` → 直接用 `sym`(Binance dapi 已经是内部格式)。**部署**:VPS crontab `*/2 cxd_live_cron.sh live` + `*/5 cxd_publish_cron.sh`;env 加 `QOUNT_RV_LIVE_ENABLE=1`/`QOUNT_RV_AUTOFUND=1`/`QOUNT_RV_UM_BUFFER_USDT=100`。**关键运维注意——carry 日频 vs 趋势 10 分钟**:① carry 腿 `_read()` 读 COIN-M 持仓走 Binance dapi 私有接口(`dapiPrivateGetPositionRisk`)，速率限制比 USDⓈ-M 严(20 vs 6000 weight/min)，cron 2 分钟一次刚好不会撞线但不要加频;② carry reconcile 在 `min_order_usdt`/`rebalance_band` 容差内自动 skip(7 合约 vs 6 目标差 $8.5<$10)，属正常态;③ `QOUNT_RV_UM_BUFFER_USDT=100`(从 200 降到 100)让 autofund 有更多空间抽 USDT 到 SPOT，但余额<$100 时 autofund 停抽——趋势腿的全钱包 sizing 保证 7 币都能交易，carry 腿是固定比例 40%×total=$148;④ BTC carry 缺位(per-pair $82 < BTC 合约面值 $100 需 $133.33 最小资金)，只跑 ETH carry 一对。**当前实态**:UMFUTURE $371(趋势 7 币做空,gate=SHUT 等 BTC 站上 200MA 开仓),SPOT $53+0.022 ETH,COIN-M 7 张 ETHUSD_260626 空;carry net delta −$33(目标 ≈0,容差内)。 |
| 2026-06-17 | ops(§21.6) | **趋势 pilot 真上线 + 三处工程修复(详见 §21.6)**。承 §21.5 现货 burn-in,owner 升级 v2 博收益(USDⓈ-M 永续 2x 逐仓 + vol_target 3% + breadth-OR + chandelier 8×)、迁全部署到 VPS(`qount-vps`)、清专用子账户、**ARMED 趋势独跑(本金 $70→充值到 $115.55)**。**🔴 prepare_swap 死锁修复(d816d76)**:Binance 对空仓账户 `fetch_positions` 返 0 行→旧 verify 永远确认不了 2x→永远拒绝交易(gate 翻多也下不了第一单);改用 **set_leverage 回显**主确认(空仓可用)、持仓回读次确认。**下单上限随本金缩放(755d9bd)**:BUY 钳到 `min(max_order_usdt, capital×leverage)`,SELL 不节流。**可达性改真实逆波动率权重(c2bd386)**:owner 指出"等权份额"不对→抽 `_inverse_vol_parity` 共享件(`target_weights` 不变)+ `natural_weights`,`unreachable_coins` 用真实目标 notional vs 交易所最小下单额;**$115.55 实测真正能下只有 BNB/SOL/XRP 三币**(BTC 目标$17.8<地板$64.8、ETH/LINK<$20、ADA<$6 全跳过),纳 BTC 需 ~$420+。看板实盘面板改永续 2x 显示 + 三类告警条。**杠杆决策**:owner 拍板 **2x cap + vt3% 维持**(加 cap 是惰性操作,真 scale 全 0.37~0.79x 无一顶 2x;真旋钮 vol_target 但收益/回撤同放大;别用杠杆补本金)。x4 共 47 单测全绿,全部 scp 部署 VPS 验证。当前 gate SHUT 空仓待 BTC 站上 200MA。 |
